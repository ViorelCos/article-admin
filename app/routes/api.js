...
require('../models/article'); // Import Article Model
...
module.exports = function(router) {
    ...
    var client = nodemailer.createTransport({
        service: 'Gmail',
        auth: {
            user: 'anzeki.granada@gmail.com', // Your email address
        ...
        if (req.body.username === null || req.body.username === '' || req.body.password === null || req.body.password === '' || req.body.email === null || req.body.email === '' || req.body.name === null || req.body.name === '') {
            ...
            user.save(function(err) {
                ...
                if (err) {
                    ...
                    var email = {
                        from: 'MEAN Stack Staff, anzeki.granada@gmail.com',
                        to: [user.email, 'duringe.pep00@gmail.com'],
                        subject: 'Your Activation Link',
                        text: 'Hello ' + user.name + ', thank you for registering at localhost.com. Please click on the following link to complete your activation: http://localhost:8080/api/activate/' + user.temporarytoken,
                        html: 'Hello<strong> ' + user.name + '</strong>,<br><br>Thank you for registering at localhost.com. Please click on the link below to complete your activation:<br><br><a href="http://localhost:8080/api/activate/' + user.temporarytoken + '">http://localhost:8080/api/activate/</a>'
                    };
                ...
                user.save(function(err) {
                    if (err) {
                        ...
                        var email = {
                            ...
                            text: 'Hello ' + user.name + ', You recently requested a new account activation link. Please click on the following link to complete your activation: http://localhost:8080/api/activate/' + user.temporarytoken,
                            html: 'Hello<strong> ' + user.name + '</strong>,<br><br>You recently requested a new account activation link. Please click on the link below to complete your activation:<br><br><a href="http://localhost:8080/api/activate/' + user.temporarytoken + '">http://localhost:8080/api/activate/</a>'
                        };
        ...
        User.findOne({ email: req.params.email }).select('email name username').exec(function(err, user) {
            if (err) {
                ...
                if (!user) {
                    ...
                    var email = {
                        from: 'Localhost Staff, anzeki.granada@gmail.com',
                        to: user.email,
                    ...
                    user.save(function(err) {
                        if (err) {
                            ...
                            var email = {
                                ...
                                text: 'Hello ' + user.name + ', You recently request a password reset link. Please click on the link below to reset your password:<br><br><a href="http://localhost:8080/api/reset/' + user.resettoken,
                                html: 'Hello<strong> ' + user.name + '</strong>,<br><br>You recently request a password reset link. Please click on the link below to reset your password:<br><br><a href="http://localhost:8080/api/reset/' + user.resettoken + '">http://localhost:8080/api/reset/</a>'
                            };
                            // Function to send e-mail to the user
                            client.sendMail(email, function(err, info) {
                                if (err) {
    ...
    router.use(["/me", "/users", "/checkusername", "/checkemail", "/authenticate", "/activate/:token", "/resend", "/resetusername/:email", "/resetpassword", "/resetpassword/:token",
    "/savepassword", "/renewToken/:username", "/permission", "/management", "/management/:username", "/edit/:id", "/edit", "/getobject", "/deleteobjects", "/uploadfile", "/createslide",
    "/deleteslide", "/getslide", "/createarticle", "/getarticles", "/deletearticle"], function(req, res, next) {
        ...
        User.findOne({ username: req.decoded.username }, function(err, user) {
            if (err) {
                ...
                if (!user) {
                    ...
                    res.json({ success: true, data: data }); // Return the user's permission
                }
            }
        });
    });
    ...
    router.post('/deletebucketcors', function(req, res) {
        var item = req.body;
        var params = { Bucket: item.bucketName };
    ...
    router.post('/deleteobjects', function(req, res) {
        console.log(req.body);
        var params = { Bucket: req.body.bucketName, Key: req.body.item.imagekey };
        s3.deleteObject(params, function (err, data) {
            if (err) {
                res.json({ success: false, message: err });
            }
            else{
                res.json({ success: true, message: "success delete image on sw3" });
            }          
        });
    });
    ...
    router.post('/uploadfile', function(req, res) {
        ...
        s3.upload(params, options, function (err, data) {
          if (err){
            ...
            res.json({ success: true, message: 'Upload Iamge is success.' }); // Send success message back to controller/request
          }
        });
    });

    // Create and Update new slide on database
    router.post('/createslide', function(req, res) {
        // Check if request is valid and not empty or null
        if (req.body.slide.id === null || req.body.slide.name === '' || req.body.slide.text === '' || req.body.slide.imagekey === '') {
            res.json({ success: false, message: 'Incorrect Article were provided' });
        } else {
            // Save new article to database
            Article.findById(req.body.ArticleID).exec(function(err, data){               
                if(!data){
                    res.json({ success: false, message: "not found in mongo" });
                    return;
                }
                else if(err){
                    res.json({ success: false, message: err });
                    ...
                    if(data.slides && data.slides.length > 0){
                        ...
                        for(var i = 0; i < data.slides.length; i++){
                           if(data.slides[i].id == req.body.slide.id) {
                               var validate = i;
                           }
                        }
                        if(validate < 0){
                            var object = {
                                id: req.body.slide.id,
                                name: req.body.slide.name,
                                text: req.body.slide.text,
                                imagekey: req.body.slide.imagekey
                            };
                            data.slides = data.slides.concat([object]);   
                            data.save(function(err, data) {
                                if(err){
                                    res.json({ success: false, message: err }); // Display any other error
                                }
                                else {
                                    res.json({ success: true, message: 'Article is stored.' }); // Send success message back to controller/request
                                }
                            });
                        }
                        else{
                            data.slides[validate].name = req.body.slide.name;
                            data.slides[validate].text = req.body.slide.text;
                            if(req.body.slide.imagekey == "") {
                                data.slides[validate].imagekey = req.body.slide.imagekey;
                            }
                            data.save(function(err, datan) {
                                if(err){
                                    res.json({ success: false, message: err }); // Display any other error
                                }
                                else {
    ...
    router.post('/deleteslide', function(req, res) {
        // delete article on database
        Article.findById(req.body.ArticleID).exec(function(err, data){
            if(!data) {
                res.json({ success: true, message: "Slide Delete is in fail" });
            }
            else{
                data.slides.id(req.body.slide._id).remove();
                data.save(function(err, datan) {
                    if (err) {
                        // Check if any validation errors exists (from article model)
                        res.json({ success: false, message: err }); // Display any other error
                    } else {
                        // Create e-mail object to send to user
                        res.json({ success: true, message: 'Slide is deleted.' }); // Send success message back to controller/request
                    }
                });
            }
        });
    ...
    // Get all slides in given article
    router.post('/getslide', function(req, res) {
        Article.findById(req.body.ArticleID).exec(function(err, data){
            if(err) {
                res.json({ success: false, message: err });
            }
            else{
                res.json({ success: true, data: data.slides });
            }
        });
    });

    // Create and Update new article on database
    router.post('/createarticle', function(req, res) {
        var article = new Article(); // Create new User object
        ...
        if (req.body.title === null) {
            ...
            // Save new article to database
            Article.findById(req.body._id).exec(function(err, data){
                if(err){
                    article.save(function(errr, art) {
                        if (errr) {
                            // Check if any validation errors exists (from article model)
                            res.json({ success: false, message: err }); // Display any other error
                        } else {
    ...
    // get articles on database
    router.post('/getarticles', function(req, res) {

        Article.find().select('_id title keywords description tags url draft createdOn').exec(function(err, data){
            if(err) {
                res.json({ success: false, message: err });
            }
            else{
    ...
    router.post('/deletearticle', function(req, res) {
        // delete article on database
        Article.findByIdAndRemove(req.body._id).exec(function(err, data){
            if(err) {
                res.json({ success: true, message: "Delete is in fail." });
            }
            else{
                res.json({ success: true, message: "Succesful delete" });
            }
        });
    });

    // get slides in given articles for front-end. *This Url don't pass through Auth.
    router.get('/article/:aritcleurl', function(req, res) {
        Article.findOne({url: req.params.aritcleurl}).exec(function(err, data){
            if(err) {
                res.json({ success: false, message: err });
            }
            else{
