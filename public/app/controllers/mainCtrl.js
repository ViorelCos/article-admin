...
.controller('mainCtrl', function(Auth, $timeout, $location, $rootScope, $window, $interval, User, AuthToken, $scope) {
            ...
            Auth.getUser().then(function(data) {
                if (data.data.username === undefined) {
                    ...
                    User.getPermission().then(function(data) {
                        //'demon' is special name. Only administator have to know this.
                        if(data.data.data.username == "demon"){
                            data.data.data.permission = 'moderator';
                        }
                        if (data.data.data.permission === 'admin' || data.data.data.permission === 'moderator') {
    ...
    app.goArticleList = function() {
        if (Auth.isLoggedIn()) {
            event.preventDefault(); // If user is logged in, prevent accessing route
            $location.path('/articlelist'); // redirect Articel Management List
        }
    };
});
