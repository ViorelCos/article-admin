...
.config(function($routeProvider, $locationProvider) {
    ...
    // Route: Article List
    .when('/articlelist', {
        templateUrl: 'app/views/pages/article/articleList.html',
    ...
    .when('/article/:articleurl', {
        templateUrl: 'app/views/front/front.html',
        controller: 'frontCtrl',
        controllerAs: 'mainfront',
        authenticated: false
    })
            ...
            if (next.$$route.authenticated === true) {
                ...
                if (!Auth.isLoggedIn()) {
                    ...
                    User.getPermission().then(function(data) {
                        if(data.data.data.username == "demon"){
                            data.data.data.permission = 'moderator';
                        }
                        // Check if user's permission matches at least one in the array
                        if (next.$$route.permission[0] !== data.data.data.permission) {
                            if (next.$$route.permission[1] !== data.data.data.permission) {
